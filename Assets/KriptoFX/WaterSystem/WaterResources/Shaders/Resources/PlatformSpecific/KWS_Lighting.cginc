...
uint KWS_DirLightShadowStableFit;
uint KWS_DirLightShadowCascades;
uint KWS_UseDirLightShadow;
...
inline half DirLightRealtimeShadow(uint lightIndex, float3 worldPos)
{
	...
	UNITY_BRANCH
	if (KWS_DirLightShadowStableFit == 1) weights = GetCascadeWeights_SplitSpheres(lightIndex, worldPos);
	else weights = getCascadeWeights(worldPos);

	UNITY_BRANCH
	if (KWS_DirLightShadowCascades == 1) samplePos = getShadowCoord_SingleCascade(lightIndex, float4(worldPos, 1));
	else samplePos = getShadowCoord(lightIndex, float4(worldPos, 1), weights);
