Shader "Hidden/KriptoFX/KWS/DynamicWaves"
{
	...
	if (i.uv[0].x < 0.01 || i.uv[0].x > 0.99 || i.uv[0].y < 0.01 || i.uv[0].y > 0.99
		...
		float prevFrame = tex2Dlod(_PrevTex, i.uv[0]).x;
		...
		float dynamicObjects = tex2D(KW_DynamicObjectsMap, i.uv[5].xy).x;
		...
		float2 maskAlphaUV = 1.0 - abs(i.uv[5].xy * 2 - 1);
