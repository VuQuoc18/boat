...
namespace KWS
{
    public class KW_DynamicWaves
    {
        ...
        public void RenderWaves(WaterSystem waterInstance, Camera cam, Vector3 camPos, Vector3 waterPos, int fps, int areaSize, int pixelsPerMeter, float propagationSpeed, float rainStrength)
        {
            ...
            if (dynamicWavesMaterial == null)
            {
                dynamicWavesMaterial = KWS_CoreUtils.CreateMaterial(dynamicWavesShaderName);
                waterInstance.AddShaderToWaterRendering(dynamicWavesMaterial);
            }

            int endIndex;
            var interactScripts = KW_WaterDynamicScripts.GetInteractScriptsInArea(InteractPos, areaSize, out endIndex);

            InteractPos = KW_Extensions.GetRelativeToCameraAreaPos(cam, camPos, areaSize, waterPos.y);
            ...
            //waterInstance.SetFloats(requireUpdateComputeShaders: false, (KWS_ShaderConstants.DynamicWaves.KW_DynamicWavesAreaSize, areaSize));
            //waterInstance.SetVectors(requireUpdateComputeShaders: false, (KWS_ShaderConstants.DynamicWaves.KW_DynamicWavesWorldPos, InteractPos));
            waterInstance.SharedData.DynamicWavesAreaPos = InteractPos;
            ...
            UpdateDynamicWavesLod(waterInstance, wavesData[0], wavesData[1], wavesData[2], areaSize, propagationSpeed, offset, InteractPos);
        ...
        void UpdateDynamicWavesLod(WaterSystem waterInstance, WavesData data1, WavesData data2, WavesData data3, int areaSize, float pixelSpeed, Vector3 offset, Vector3 areaPos)
        {
            ...
            waterInstance.SharedData.DynamicWavesAreaPos = areaPos;
            waterInstance.SharedData.DynamicWavesAreaSize = areaSize;
            waterInstance.SharedData.DynamicWaves = target.DataRT.rt;
            waterInstance.SharedData.DynamicWavesNormal = target.NormalRT.rt;
