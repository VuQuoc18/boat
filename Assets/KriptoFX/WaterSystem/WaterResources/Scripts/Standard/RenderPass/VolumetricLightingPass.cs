...
namespace KWS
{
    public class VolumetricLightingPass: WaterPass
    {
        ...
        public void CollectLightsToBuffer(Camera cam, WaterSystem water)
        {
        ...
        {
            var frustumCameraPlanes = WaterSystem.CurrentCameraFrustumPlanes;
            var lights = KWS_WaterLights.Lights;
            ...
            if (activeDirLights.Count > 0)
            {
                ...
                Shader.SetGlobalInt(LightsID.KWS_DirLightShadowStableFit, isSplit ? 1 : 0);
                Shader.SetGlobalInt(LightsID.KWS_DirLightShadowCascades,  QualitySettings.shadowCascades);
                Shader.SetGlobalInt(LightsID.KWS_UseDirLightShadow, useShadows ? 1 : 0);
            }
        ...
        {
            if (_cmd != null && _lastCamera != null)
            {
                _lastCamera.RemoveCommandBuffer(_camEvent, _cmd);
            }
        }
