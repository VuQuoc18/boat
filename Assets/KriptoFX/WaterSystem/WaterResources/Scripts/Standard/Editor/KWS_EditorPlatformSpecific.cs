...
namespace KWS
{
    public partial class KWS_Editor
    {
        ...
        {
            if (_waterSystem.Settings.UseVolumetricLight && KWS_WaterLights.Lights.Count == 0) EditorGUILayout.HelpBox("Water->'Volumetric lighting' doesn't work because no lights has been added for water rendering! Add the script 'AddLightToWaterRendering' to your light.", MessageType.Error);
        }

        void CheckPlatformSpecificMessages_Reflection()
        {
            if (_waterSystem.Settings.ReflectSun)
            {
